...
if __name__ == "__main__":
    # We probe the parent IDs of 128:6
    # Let's try 128:5 down to 128:1, and also 128 (if valid)
    parent_candidates = ["128:5", "128:4", "128:3", "128:2", "128:1", "128"]
    req_id = 9000
    for parent in parent_candidates:
        print(f"Probing candidate {parent}...")
        res = fetch_metadata(parent, req_id)
        req_id += 1
        if res and "error" not in res:
            ...
            print(f"SUCCESS for {parent}: {text_val[:500]}")
        else:
            print(f"Failed for {parent}")
        time.sleep(0.2)
